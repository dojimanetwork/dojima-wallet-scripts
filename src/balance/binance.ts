...
import { Fees as BinanceFees, isTransferFee } from "../util/binance";
...
export default class BinanceChain extends BinanceAccount {
  mnemonic: string;
    ...
    async transfer(amount:number,recepient:string,assest:string,memo?:string){
      await this._client.initChain()
      const fromAccount =  this._client.recoverAccountFromMnemonic(this.mnemonic);
    ...
    // async createTranscation(amount:number,recepient:string,assest:string,memo?:string,sequence: number | null = null
    //   ){
    //   await this._client.initChain()
    //   const fromAccount =  this._client.recoverAccountFromMnemonic(this.mnemonic);
    //   await this._client.setPrivateKey(fromAccount.privateKey)
