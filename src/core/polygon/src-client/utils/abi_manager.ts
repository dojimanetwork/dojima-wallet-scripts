...
import { resolve, promiseResolve } from "./index";
...
export class ABIManager {
  ...
  init() {
    return service.abi
      .getAddress(this.networkName, this.version)
      .then((result) => {
  ...
  getConfig(path: string) {
    return resolve(cache[this.networkName][this.version].address, path);
  }

  getABI(contractName: string, bridgeType = "plasma"): Promise<any> {
    const targetBridgeABICache =
      cache[this.networkName][this.version].abi[bridgeType];
    ...
    return service.abi
      .getABI(this.networkName, this.version, bridgeType, contractName)
      .then((result) => {
