import { Web3SideChainClient } from "./index";
import { ExitUtil } from "../pos";
import { BaseToken, utils } from "../index";
...
export class BridgeClient<T> {
  ...
  isDeposited(depositTxHash: string) {
    ...
    const token = new BaseToken(
      {
        address: client.abiManager.getConfig(
    ...
    return token
      ...
      .then((contract) => {
        return Promise.all([
          ...
          token["processRead"]<string>(contract.method("lastStateId")),
        ]);
      })
      .then((result) => {
        ...
        const targetLog = receipt.logs.find(
          (q) => q.topics[0] === eventSignature
        );
        if (!targetLog) {
          throw new Error("StateSynced event not found");
        }
        const rootStateId = client.child.decodeParameters(targetLog.topics[1], [
          "uint256",
        ])[0];
        const rootStateIdBN = utils.BN.isBN(rootStateId)
          ? rootStateId
          : new utils.BN(rootStateId);
        return new utils.BN(lastStateId).gte(rootStateIdBN);
      });
  }
}
