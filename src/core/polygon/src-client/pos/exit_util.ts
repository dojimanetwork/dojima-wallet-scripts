...
import { ERROR_TYPE, IBaseClientConfig, IRootBlockInfo, utils } from "../index";
...
export class ExitUtil {
  private dojimaClient_: BaseWeb3Client;
  ...
  constructor(
    ...
    this.dojimaClient_ = client.child;
  ...
  getChainBlockInfo(burnTxHash: string) {
    return Promise.all([
      ...
      this.dojimaClient_.getTransaction(burnTxHash),
    ]).then((result) => {
      return {
  ...
  isCheckPointed(burnTxHash: string) {
    return this.getChainBlockInfo(burnTxHash).then((result) => {
      return this.isCheckPointed_(result);
    });
  }
  ...
  private getRootBlockInfo(txBlockNumber: number) {
    ...
    return this.rootChain
      .findRootBlockFromChild(txBlockNumber)
      .then((blockNumber) => {
  ...
  private getRootBlockInfoFromAPI(txBlockNumber: number) {
    this.dojimaClient_.logger.log("block info from API 1");
    return service.network
      .getBlockIncluded(this.config.network, txBlockNumber)
      .then((headerBlock) => {
        this.dojimaClient_.logger.log("block info from API 2", headerBlock);
        if (
          !headerBlock ||
          !headerBlock.start ||
          !headerBlock.end ||
      ...
      .catch((err) => {
        this.dojimaClient_.logger.log("block info from API", err);
  ...
  private getBlockProofFromAPI(
    ...
    return service.network
      ...
      .then((blockProof) => {
        ...
        this.dojimaClient_.logger.log("block proof from API 1");
        return blockProof;
      })
      .catch((_) => {
  ...
  buildPayloadForExit(
    ...
    return this.getChainBlockInfo(burnTxHash)
      .then((blockInfo) => {
        ...
        return Promise.all([
          this.dojimaClient_.getTransactionReceipt(burnTxHash),
          this.dojimaClient_.getBlockWithTransaction(txBlockNumber),
        ]);
      })
      .then((result) => {
        [receipt, block] = result;
        return isFast
          ? this.getRootBlockInfoFromAPI(txBlockNumber)
      ...
      .then((rootBlockInfoResult) => {
        ...
        // build block proof
        return isFast
          ? this.getBlockProofFromAPI(txBlockNumber, rootBlockInfo)
  ...
  getExitHash(burnTxHash, logEventSig) {
    ...
    return Promise.all([
      ...
      .then((result) => {
        lastChildBlock = result[0];
        receipt = result[1];
        return this.dojimaClient_.getBlockWithTransaction(receipt.blockNumber);
      })
      .then((blockResult) => {
        block = blockResult;
        if (
          !this.isCheckPointed_({
            lastChildBlock: lastChildBlock,
            txBlockNumber: receipt.blockNumber,
          })
        ) {
          this.dojimaClient_.logger
            .error(ERROR_TYPE.BurnTxNotCheckPointed)
            .throw();
        }
        return ProofUtil.getReceiptProof(
          receipt,
      ...
      .then((receiptProof: any) => {
        ...
        return this.dojimaClient_.etheriumSha3(
          receipt.blockNumber,
          ethUtils.bufferToHex(Buffer.concat(nibbleArr)),
          logIndex
        );
      });
  }
