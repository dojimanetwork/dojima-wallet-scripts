...
import { IPOSClientConfig, MAX_AMOUNT } from "../index";
...
export class ERC20 extends POSToken {
  ...
  getAllowance(userAddress: string, option?: ITransactionOption) {
    ...
    return Promise.all([this.getPredicateAddress(), this.getContract()]).then(
      (result) => {
  ...
  approveMax(option?: ITransactionOption) {
    return this.approve(MAX_AMOUNT, option);
  }
  ...
  private depositEther_(
    ...
    return this.rootChainManager
      .method("depositEtherFor", userAddress)
      .then((method) => {
        return this.processWrite(method, option);
      });
  }
  ...
  withdrawStart(amount: TYPE_AMOUNT, option?: ITransactionOption) {
    ...
    return this.getContract().then((contract) => {
      const method = contract.method("withdraw", Converter.toHex(amount));
      return this.processWrite(method, option);
    });
  }
