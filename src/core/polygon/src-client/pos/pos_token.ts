...
export class POSToken extends BaseToken<IPOSClientConfig> {
  ...
  getPredicateAddress(): Promise<string> {
    ...
    return this.rootChainManager
      .method("tokenToType", this.contractParam.address)
      .then((method) => {
        return method.read();
      })
      .then((tokenType) => {
        ...
        return this.rootChainManager.method("typeToPredicate", tokenType);
      })
  ...
  protected isWithdrawn(txHash: string, eventSignature: string) {
    ...
    return this.exitUtil
      .getExitHash(txHash, eventSignature)
      .then((exitHash) => {
        return this.rootChainManager.isExitProcessed(exitHash);
      });
  }

  protected withdrawExitPOS(
    burnTxHash: string,
    ...
    return this.exitUtil
      .buildPayloadForExit(burnTxHash, eventSignature, isFast)
      .then((payload) => {
