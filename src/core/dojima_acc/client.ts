...
function d11kFaucet(address: string) {
    axios.post(`https://faucet.h4s.dojima.network/api/v1/D11K/${address}`).then(() => {
        return
    })
}

function h4sFaucet(address: string) {
    axios.post(`https://faucet.h4s.dojima.network/api/v1/H4S/${address}`).then(() => {
        return
    })
}
...
const hermesLoop = async () => {
    ...
    if (Number(balance) <= 0.02) {
        ...
        try {
            h4sFaucet(accounts[randomArrayIndex].hermes_address)
        } catch (e) {
            hermesLoop()
        }
    }
    if ((Number(balance) - 0.02) > 0) {
        ...
        try {
            ...
            const hermesClient = new HermesSdkClient({phrase: acc.mnemonic.phrase, network: Network.Testnet});
                            ...
                            provider.getBlock(block).then(async () => {
                                ...
                                if (Number(sourceBalance.sub(txBuffer)) > 0) {
                                    ...
                                    if(Number(remainingAmount) >= Number(ethers.utils.parseEther('1.5'))) {
                                        ...
                                        targetAddress = accounts[targetRandomArrayIndex].dojima_address;
                                        const randomPercent = pickRandomNumber(50, 80) / 100
