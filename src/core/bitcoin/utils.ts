 ...
 */
export const haskoinbroadcastTx = async ({ haskoinUrl, txHex }: BroadcastTxParams): Promise<TxHash> => {
    return await haskoinApi.broadcastTx({ haskoinUrl, txHex })
}

export const sochainbroadcastTx = async ({ sochainUrl, txHex, network }: TxBroadcastParams): Promise<TxHash> => {
    return await sochain.broadcastTx({txHex, sochainUrl, network })
}
