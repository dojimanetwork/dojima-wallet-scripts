...
import { Converter, merge } from "./index";
...
export class BaseToken<T_CLIENT_CONFIG> {
  ...
  getContract(): Promise<BaseContract> {
    ...
    return this.client
      .getABI(contractParam.name, contractParam.bridgeType)
      .then((abi) => {
  ...
  protected processWrite(
    ...
    return this.createTransactionConfig({
      ...
      const methodResult = method.write(config);
      return methodResult;
    });
  }
  ...
  protected sendTransaction(
    ...
    return this.createTransactionConfig({
      ...
      const methodResult = client.write(config);
      return methodResult;
    });
  }
  ...
  protected getClient(isParent: boolean) {
    return isParent ? this.client.parent : this.client.child;
  }

  private getContract_({ isParent, tokenAddress, abi }: getContractParams) {
    const client = this.getClient(isParent);
  ...
  protected createTransactionConfig({
    txConfig,
    method,
    isParent,
    isWrite,
  }: ITransactionConfigParam) {
    const defaultConfig = isParent
      ? this.parentDefaultConfig
      : this.childDefaultConfig;
    txConfig = merge(defaultConfig, txConfig || {});
    const client = isParent ? this.client.parent : this.client.child;
    client.logger.log(
      "txConfig",
      txConfig,
      "onRoot",
    ...
    const estimateGas = (config: ITransactionRequestConfig) => {
      return method ? method.estimateGas(config) : client.estimateGas(config);
    };
  ...
  protected transferERC20(
    to: string,
    amount: TYPE_AMOUNT,
    option?: ITransactionOption
  ) {
    return this.getContract().then((contract) => {
      const method = contract.method("transfer", to, Converter.toHex(amount));
      return this.processWrite(method, option);
    });
  }

  protected transferERC721(
    from: string,
    to: string,
    tokenId: string,
    option: ITransactionOption
    ...
    return this.getContract().then((contract) => {
      const method = contract.method("transferFrom", from, to, tokenId);
      return this.processWrite(method, option);
    });
  }
