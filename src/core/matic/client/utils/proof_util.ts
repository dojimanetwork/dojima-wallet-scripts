...
import { Converter, promiseResolve} from "../index";
...
export class ProofUtil {
  ...
  static buildBlockProof(
      maticWeb3: BaseWeb3Client,
    startBlock: number,
    endBlock: number,
    blockNumber: number
  ) {
  ...
  static getReceiptProof(
    ...
    if (!receiptsVal) {
      ...
      block.transactions.forEach((tx) => {
        ...
        receiptPromises.push(web3.getTransactionReceipt(tx.transactionHash));
      });
      ...
      .then((receipts) => {
        return Promise.all(
          receipts.map((siblingReceipt) => {
            const path = ethUtils.rlp.encode(siblingReceipt.transactionIndex);
      ...
      .then((_) => {
        ...
        return new Promise((resolve, reject) => {
          receiptsTrie.findPath(
              ethUtils.rlp.encode(receipt.transactionIndex),
            (err: any, rawReceiptNode: { value: Buffer; }, reminder: string | any[], stack: any[]) => {
              if (err) {
                return reject(err);
              }
              ...
              const prf = {
                ...
                path: ethUtils.rlp.encode(receipt.transactionIndex),
                value: ProofUtil.isTypedReceipt(receipt)
                  ? rawReceiptNode.value
                  : ethUtils.rlp.decode(rawReceiptNode.value),
              };
              resolve(prf);
            }
          );
  ...
  static getStateSyncTxHash(block: IBlockWithTransaction): Buffer {
    return ethUtils.keccak256(
      Buffer.concat([
  ...
  static getReceiptBytes(receipt: ITransactionReceipt) {
    let encodedData = ethUtils.rlp.encode([
      ethUtils.toBuffer(
