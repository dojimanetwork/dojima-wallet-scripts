...
export abstract class BaseWeb3Client {
  ...
  abstract getBlock(blockHashOrBlockNumber: string | number): Promise<IBlock>;
  abstract getBlockWithTransaction(
  ...
  abstract sendRPCRequest(
    request: IJsonRpcRequestPayload
  ...
  abstract etheriumSha3(...value: (string | number)[]): string;
}
