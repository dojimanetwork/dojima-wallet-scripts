...
export class ExitUtil {
  ...
  private getBlockProof(txBlockNumber: number, rootBlockInfo: { start: string; end: string }) {
  ...
  private getBlockProofFromAPI(
    txBlockNumber: number,
    rootBlockInfo: { start: string; end: string }
  ) {
    return service.network
      .getProof(
        this.config.network,
  ...
  private encodePayload_(
    headerNumber: ethUtils.rlp.Input,
    buildBlockProof: ethUtils.rlp.Input,
    blockNumber: ethUtils.rlp.Input,
    timestamp: ethUtils.rlp.Input,
    transactionsRoot: Buffer,
    receiptsRoot: Buffer,
    receipt: Buffer,
    receiptParentNodes: ethUtils.rlp.Input,
    path: Uint8Array,
    logIndex: ethUtils.rlp.Input
  ) {
  ...
  getExitHash(burnTxHash: string, logEventSig: string) {
    ...
    return Promise.all([
      ...
      .then((result) => {
        lastChildBlock = result[0];
        receipt = result[1];
        return this.maticClient_.getBlockWithTransaction(receipt.blockNumber);
      })
      .then((blockResult) => {
        block = blockResult;
        if (
      ...
      .then((receiptProof: any) => {
        const logIndex = this.getLogIndex_(logEventSig, receipt);
        const nibbleArr: Uint8Array[] | Buffer[] = [];
