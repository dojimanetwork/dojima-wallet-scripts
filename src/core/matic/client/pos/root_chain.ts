...
import { BaseBigNumber } from "../index";
...
export class RootChain extends BaseToken<IPOSClientConfig> {
  ...
  method(methodName: string, ...args: (string | number | BaseBigNumber)[]) {
  ...
  async findRootBlockFromChild(
    childBlockNumber: TYPE_AMOUNT
  ): Promise<BaseBigNumber> {
    ...
    while (start.lte(end)) {
      ...
      const headerBlock = await headerBlocksMethod.read<{
        start: number;
        end: number;
      }>();
