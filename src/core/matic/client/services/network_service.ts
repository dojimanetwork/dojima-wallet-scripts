import { BaseBigNumber, utils } from "../index";
...
export class NetworkService {
  ...
  getBlockIncluded(network: string, blockNumber: number) {
    ...
    return this.httpRequest
      ...
      .then((result) => {
        ...
        const decimalHeaderBlockNumber =
          headerBlockNumber.slice(0, 2) === "0x"
            ? parseInt(headerBlockNumber, 16)
            : headerBlockNumber;
  ...
  getProof(network: string, start: string, end: string, blockNumber: number) {
