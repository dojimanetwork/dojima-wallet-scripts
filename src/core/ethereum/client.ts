...
import { Address, Asset, AssetETH, BaseAmount, Chain, assetToString, baseAmount, delay } from '@d11k-ts/utils'
 ...
 */
class Client extends BaseChainClient implements ChainClient, EthereumClient {
    ...
    setupProviders(): void {
        ...
        if(this.etherscanApiKey) {
            const testnetProvider = new ethers.providers.EtherscanProvider(EthNetwork.Test, this.etherscanApiKey)
            const mainnetProvider = new ethers.providers.EtherscanProvider(EthNetwork.Main, this.etherscanApiKey)
            this.providers.set(Network.Testnet, testnetProvider)
            this.providers.set(Network.Mainnet, mainnetProvider)
            this.providers.set(Network.Stagenet, mainnetProvider)
        }
    ...
    async estimateGasPrices(): Promise<GasPrices> {
        try {
            ...
            const ratesInGwei: FeeRates = standardFeeRates(await this.getFeeRateFromHermeschain())
            return {
