...
import { EthWeb3 } from "../src/types/interfaces/web3";

async function getInit() {
    const web3Instance = await new EthWeb3('letter ethics correct bus asset pipe tourist vapor envelope kangaroo warm dawn').init();
    const ethereumResult = new chains.EthereumChain(web3Instance.web3, web3Instance.walletDetails);
    const balance = ethereumResult.getBalance();
    console.log(balance);
